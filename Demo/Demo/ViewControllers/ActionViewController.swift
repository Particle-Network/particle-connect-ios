...
class ActionViewController: UIViewController {
    ...
    let auth = ParticleAuthCore.Auth()
    ...
    override func viewDidLoad() {
        ...
        (adapter as? WalletConnectAdapter)?.extendConnection(publicAddress: getSender())
    }

    @IBAction func signAndSendTransactionNative() {
        return getTransactionNative().flatMap { [weak self] transaction in
    ...
    @IBAction func signMessage() {
        ...
        sourceTextView.text = message
        adapter.signMessage(publicAddress: getSender(), message: message).subscribe { [weak self] result in
            guard let self = self else { return }
            switch result {
            ...
            case let .success(signedMessage):
                print(signedMessage)
    ...
    @IBAction func signMessageHexString() {
        resultTextView.text = ""
        let message = "0x" + getMessage().data(using: .utf8)!.map { String(format: "%02x", $0) }.joined()
        sourceTextView.text = message
        adapter.signMessage(publicAddress: getSender(), message: message).subscribe { [weak self] result in
            guard let self = self else { return }
            switch result {
    ...
    @IBAction func signTypedDataHexString() {
        resultTextView.text = ""
        let typedData = "0x" + getTypedDataV4().data(using: .utf8)!.map { String(format: "%02x", $0) }.joined()
        sourceTextView.text = typedData
        adapter.signTypedData(publicAddress: getSender(), data: typedData).subscribe { [weak self] result in
            guard let self = self else { return }
            switch result {
    ...
    @IBAction func loginSignInWithEthereum() {
        ...
        adapter.signInWithEthereum(config: message, publicAddress: getSender()).subscribe { [weak self] result in
            guard let self = self else { return }
            switch result {
            case let .failure(error):
                print(error)
            ...
            case .success(let (sourceMessage, signature)):
                print("sourceMessage = \(signature), \n\nsignature = \(signature)")
                self.resultTextView.text = signature
    ...
    @IBAction func verify() {
        resultTextView.text = ""
        guard !message.isEmpty, !signature.isEmpty else {
            print("message or signature is empty")
            return
        }

        let siwe = try! SiweMessage(message)

        var against = signature
...
extension ActionViewController {
    private func isSolana() -> Bool {
        if (connectWalletModel.walletType == .authCore && ParticleNetwork.getChainInfo().isSolanaChain) || connectWalletModel.walletType == .solanaPrivateKey || connectWalletModel.walletType == .phantom {
            return true
        } else {
            return false
        }
    ...
    private func getSender() -> String {
        if connectWalletModel.walletType == .authCore {
    ...
    private func getTransactionNative() -> Single<String> {
        if isSolana() {
            return getRecentBlockHash().map { [weak self] blockhash in
                guard let self = self else { return "" }
                let instruction = SystemProgram.transferInstruction(from: try! PublicKey(string: self.getSender()), to: try! PublicKey(string: self.getReceiver()), lamports: 100_000)
                ...
                let transaction = Base58.encode(try! solanaTransaction.serialize(requiredAllSignatures: false, verifySignatures: false))
                return transaction
            }
        } else {
            return ParticleWalletAPI.getEvmService().createTransaction(from: getSender(), to: "0x16380a03f21e5a5e339c15ba8ebe581d194e0db3", value: BInt(123_456).toHexString(), data: "0x")
        }
    }

    private func getTransactionToken() -> Single<String> {
        if isSolana() {
            return ParticleWalletAPI.getSolanaService().serializeTransaction(type: .transferToken, sender: getSender(), receiver: "9LR6zGAFB3UJcLg9tWBQJxEJCbZh2UTnSU14RBxsK1ZN", lamports: BInt(1_000_000_000), mintAddress: "GobzzzFQsFAHPvmwT42rLockfUCeV3iutEkK218BxT8K", payer: nil).map {
                $0.stringValue
            }
        } else {
            if ParticleNetwork.getChainInfo().isTron {
                ...
                let params = ContractParams.erc20Transfer(contractAddress: "0x37349aeb75a32f8c4c090daff376cf975f5d2eba", to: "0x0cF3Ffe33E45ad43fcd0aa7016C590b5F629d9AA", amount: 1_000_000_000_000_000_000)
                ...
                let params = ContractParams.erc20Transfer(contractAddress: "0x779877A7B0D9E8603169DdbD7836e478b4624789", to: "0x0cF3Ffe33E45ad43fcd0aa7016C590b5F629d9AA", amount: 10_000_000_000_000_000)
                return ParticleWalletAPI.getEvmService().writeContract(contractParams: params, from: getSender())
            }
        }
    }
...
//            return ""
            return Base58.encode("Hello Particle".data(using: .utf8)!)
        } else {
    ...
    private func getTypedDataV4() -> String {
        let typedData = """
        {\"types\":{\"Maker\":[{\"name\":\"quoteType\",\"type\":\"uint8\"},{\"name\":\"globalNonce\",\"type\":\"uint256\"},{\"name\":\"subsetNonce\",\"type\":\"uint256\"},{\"name\":\"orderNonce\",\"type\":\"uint256\"},{\"name\":\"strategyId\",\"type\":\"uint256\"},{\"name\":\"collectionType\",\"type\":\"uint8\"},{\"name\":\"collection\",\"type\":\"address\"},{\"name\":\"currency\",\"type\":\"address\"},{\"name\":\"signer\",\"type\":\"address\"},{\"name\":\"startTime\",\"type\":\"uint256\"},{\"name\":\"endTime\",\"type\":\"uint256\"},{\"name\":\"price\",\"type\":\"uint256\"},{\"name\":\"itemIds\",\"type\":\"uint256[]\"},{\"name\":\"amounts\",\"type\":\"uint256[]\"},{\"name\":\"additionalParameters\",\"type\":\"bytes\"}],\"EIP712Domain\":[{\"name\":\"name\",\"type\":\"string\"},{\"name\":\"version\",\"type\":\"string\"},{\"name\":\"chainId\",\"type\":\"uint256\"},{\"name\":\"verifyingContract\",\"type\":\"address\"}]},\"domain\":{\"name\":\"LooksRareProtocol\",\"version\":\"2\",\"chainId\":"\(getChainId())",\"verifyingContract\":\"0x0d675e274a4269636d13dade7ecc16df8a7775dd\"},\"primaryType\":\"Maker\",\"message\":{\"quoteType\":\"1\",\"globalNonce\":\"0\",\"subsetNonce\":\"0\",\"orderNonce\":\"4\",\"strategyId\":\"0\",\"collectionType\":\"0\",\"collection\":\"0x96b80d19a85cd434b5d14482985b76896c26cff3\",\"currency\":\"0x0000000000000000000000000000000000000000\",\"signer\":\"0x4f767d46bd330e921c206e72f5002f9e5c62281b\",\"startTime\":\"1690892448\",\"endTime\":\"2006252448\",\"price\":\"1000000000000000000\",\"itemIds\":[\"367722542\"],\"amounts\":[\"1\"],\"additionalParameters\":\"0x\"}}
        """
...
extension ActionViewController {
    func getRecentBlockHash() -> Single<String> {
        let request: Single<Response<RecentBlockHash>> = nodeService.request(method: "getRecentBlockhash")
        return request.map {
...
//        """
    ...
    func testEvmPrivateKey() {
        let from = "0x2648cfE97e33345300Db8154670347b08643570b"
        let transaction = "7b0d0a2264617461223a20223078222c0d0a2266726f6d223a2022307832363438636645393765333333343533303044623831353436373033343762303836343335373062222c0d0a22746f223a22307832363438636645393765333333343533303044623831353436373033343762303836343335373062220d0a7d"
